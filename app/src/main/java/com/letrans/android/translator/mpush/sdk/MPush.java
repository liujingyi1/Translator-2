...
public final class MPush {
    ...
    private static final String TAG = "RTranslator/MPush";
    ...
    MPushService mPushService = null;
    List<HttpClientListener> mHttpClientListener = new ArrayList<HttpClientListener>();
    ...
    public void init(Context context) {
        ...
        mServiceIntent = new Intent(ctx, MPushService.class);
    }
    ...
    public void startPush() {
        if (hasInit()) {
            ...
            ctx.bindService(mServiceIntent,
    ...
    public void setHttpClientListener(HttpClientListener httpClientListener) {
        ...
        if (!mHttpClientListener.contains(httpClientListener)) {
            mHttpClientListener.add(httpClientListener);
        }
        if (mPushService != null) {
            mPushService.addHttpClientListener(httpClientListener);
        }
    }

    public void removeHttpClientListener(HttpClientListener httpClientListener) {
        if (mHttpClientListener.contains(httpClientListener)) {
            mHttpClientListener.remove(httpClientListener);
        }
        if (mPushService != null) {
            mPushService.removeHttpClientListener(httpClientListener);
        }
    }
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName name) {
            mPushService = null;
            mPushService.removeAllHttpClientListener();
        }

        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            Logger.i(TAG, "onServiceConnected");
            mPushService = ((MPushService.PushServiceBinder) service).getService();
            for (HttpClientListener listener:mHttpClientListener) {
                mPushService.addHttpClientListener(listener);
            }
        }
    };
}
