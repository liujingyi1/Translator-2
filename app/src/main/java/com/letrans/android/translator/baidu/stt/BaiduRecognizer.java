...
public class BaiduRecognizer {
    private static final String TAG = "RTranslator/BaiduRecognizer";

    private static BaiduRecognizer mInstance;

    public static BaiduRecognizer getInstance(Context context, IRecogListener recogListener) {
        if (mInstance == null) {
            synchronized (BaiduRecognizer.class) {
                if (mInstance == null) {
                    mInstance = new BaiduRecognizer(context, recogListener);
                }
            }
        }
     ...
     */
    public BaiduRecognizer(Context context, IRecogListener recogListener) {
    ...
    public BaiduRecognizer(Context context, EventListener eventListener) {
        if (isInited) {
            Logger.e(TAG, "还未调用release()，请勿新建一个新类");
    ...
    public void loadOfflineEngine(Map<String, Object> params) {
        ...
        Logger.i(TAG + ".Debug", "loadOfflineEngine params:" + json);
    ...
    public void start(Map<String, Object> params) {
        String json = new JSONObject(params).toString();
        Logger.i(TAG + ".Debug", "asr params(识别参数，反馈请带上此行日志):" + json);
