...
public class CustomAndroidAudioGenerator implements BaseAudioGenerator {
    ...
    private static final String TAG = "RTranslator/CustomAndroidAudioGenerator";
    ...
    private Runnable mReadTask = new Runnable() {
        ...
        private void pushDataQueueLocked() {
            ...
            if (!mRecorderData.offer(data)) {
                Logger.d(TAG, "[pushDataQueueLocked]: too slow to take data. clear all.");
                mRecorderData.clear();
        ...
        @Override
        public void run() {
            Logger.d(TAG, "RooboRecorder begin recording... + mMinBufferSize=" + mRecBufSize);
            ...
            while (!mFinished) {
                ...
                if (len <= 0) {
                    Logger.e(TAG, "read error,lastWriteRemain:" + lastWriteRemain + " len:" + len);
                    continue;
                }
                int pos = 0;
                ...
                if (lastWriteRemain > 0) {
                    ...
                    if (new_frame_need > len) {
                        Logger.d(TAG, "lastWriteRemain:" + lastWriteRemain + " len:" + len);
...
//                    mAudioSource.process(mWriteDataBuf, FRAME_SIZE);
            ...
            Logger.d(TAG, "RooboRecorder finish recording...");
        }
    };
    ...
    synchronized public void onStart() {
        if (mWorkerThread != null || mWorkerHandler != null) {
            Logger.e(TAG, "[onStart]: recorder has been started!!!");
        }
        mFinished = false;
